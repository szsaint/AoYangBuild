...
- (void)viewDidLoad {
    ...
    self.title =@"我的二维码";
    _str =[[NSUserDefaults standardUserDefaults]objectForKey:@"uid"];
...
//   
    ...
    //二维码
    UIView *view = [[UIView alloc]initWithFrame:CGRectMake( (CGRectGetWidth(self.view.frame)-CGRectGetWidth(self.view.frame)*5/6)/2, (self.view.frame.size.height-CGRectGetWidth(self.view.frame)*5/6)/2, CGRectGetWidth(self.view.frame)*5/6, CGRectGetWidth(self.view.frame)*5/6)];
...
- (void)createQR1
{
    ...
    NSString *str =[[NSUserDefaults standardUserDefaults]objectForKey:@"uid"];
     UIImage *qrImg = [LBXScanWrapper createQRWithString:[NSString stringWithFormat:@"澳洋集团的二维码：%@",str] size:_qrImgView.bounds.size];
...
- (void)createQR2
{
    ...
    UIImage *image = [LBXScanWrapper createQRWithString:_str size:_qrImgView.bounds.size];
    //二维码上色
...
- (void)createQR3
{
    ...
    _qrImgView.image = [LBXScanWrapper createQRWithString:_str
